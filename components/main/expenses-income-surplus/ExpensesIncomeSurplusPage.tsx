...
import { AlarmClock, FileSpreadsheet, FileText, Search, List } from "lucide-react";
import { loadReportsForIncomeAndExpenses } from "@/lib/services/expenses-income-surplus.service";
import ExpensesIncomeSurplusDetailsDialog from "./ExpensesIncomeSurplusDetailsDialog";
...
export default function ExpensesIncomeSurplusPage() {
  ...
  async function loadReports() {
    ...
    setNameFilter("");
    try {
      const resp = await loadReportsForIncomeAndExpenses({ year, month });
      const data = resp?.[0] ? Object.values(resp[0]) : [];
      setReports(data.map(normalizeReport));
    } catch (err: any) {
      toast.current?.show({ severity: "error", summary: "No se pudo cargar los minutos excedentes", detail: err.message });
    } finally {
      setIsLoading(false);
    }
  }
  ...
  async function exportExcel() {
    ...
    const headers = rows.length > 0 ? Object.keys(rows[0]) : ["Agente", "Tiempo total", "Tiempo excedido"];

    const workbook = new ExcelJS.Workbook();
    const worksheet = workbook.addWorksheet("Llegadas tardes");
    ...
    link.href = url;
    link.download = `Llegadas tardes de ${MONTH_NAMES[month - 1]} de ${year}.xlsx`;
    link.click();
  ...
  async function exportPdf() {
    ...
    try {
      ...
      doc.text(`Llegadas tardes de ${MONTH_NAMES[month - 1]} de ${year}`, textX, 23.5);
      ...
      autoTable(doc, {
        startY: 35,
        head: [["Agente", "Tiempo total", "Tiempo excedido"]],
  ...
  return (
    <>
      ...
      <div className="fadeIn animated">
        ...
        <div className="card profile-card">
          <div className="d-flex align-items-center flex-wrap px-3 pt-3 pb-3" style={{ gap: "12px" }}>
            <div style={{ width: 38, height: 38, borderRadius: "11px", background: "#fef9c3", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
              <AlarmClock size={18} color="#eab308" />
            </div>
            <div className="flex-grow-1">
              <h5 className="mb-0 font-weight-bold" style={{ fontSize: "0.93rem", color: "#1e293b" }}>Llegadas tardes</h5>
              <small style={{ color: "#94a3b8", fontSize: "0.75rem" }}>Tiempo excedido de llegada por agente</small>
            </div>
        ...
        <div className="card profile-card mt-4">
          <div className="d-flex align-items-center px-3 pt-3 pb-2" style={{ gap: "12px" }}>
            ...
            <div className="flex-grow-1">
              <h5 className="mb-0 font-weight-bold" style={{ fontSize: "0.93rem", color: "#1e293b" }}>Buscar llegadas tardes</h5>
        ...
        <div className="card profile-card mt-4">
          <div className="d-flex align-items-center px-3 pt-3 pb-2" style={{ gap: "12px" }}>
            ...
            <div className="flex-grow-1">
              <h5 className="mb-0 font-weight-bold" style={{ fontSize: "0.93rem", color: "#1e293b" }}>Listado de llegadas tardes</h5>
              <small style={{ color: "#94a3b8", fontSize: "0.75rem" }}>Tiempo total y tiempo excedido por agente</small>
            </div>
            <button
              type="button"
              disabled={isLoading}
          ...
          <div className="card-body" style={{ padding: "16px 20px 20px" }}>
            ...
            >
              ...
              <Column header="TIEMPO TOTAL" sortable field="minutesInNumber" style={{ width: "16%" }} body={(item) => <small className="p-1">{item.minutes}</small>} />
              <Column
                ...
                style={{ width: "16%" }}
                body={(item) => {
                  const colors = item.exceededInNumber > 0 ? EXCEEDED_COLORS.danger : EXCEEDED_COLORS.muted;
                  return (
        ...
        header={
          <div className="d-flex align-items-center" style={{ gap: "12px" }}>
            ...
            <div style={{ minWidth: 0 }}>
              <p className="mb-0 font-weight-bold" style={{ fontSize: "0.93rem", color: "#1e293b" }}>Llegadas tardes</p>
      ...
      >
        {pdfUrl && <iframe src={pdfUrl} style={{ flex: 1, width: "100%", border: "none" }} title="Llegadas tardes PDF" />}
      </Sidebar>

      <ExpensesIncomeSurplusDetailsDialog
