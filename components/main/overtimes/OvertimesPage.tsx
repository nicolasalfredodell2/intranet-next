...
export default function OvertimesPage() {
  ...
  async function loadReports() {
    ...
    setNameFilter("");
    try {
      const resp = await loadOvertimes(year, month);
      const data = Array.isArray(resp) ? resp : [];
      setReports(data.map(normalizeReport));
    } catch (err: any) {
      toast.current?.show({ severity: "error", summary: "No se pudieron cargar las overtimes", detail: err.message });
    } finally {
      setIsLoading(false);
    }
  }
  ...
  async function exportExcel() {
    ...
    link.href = url;
    link.download = `Overtimes de ${MONTH_NAMES[month - 1]} de ${year}.xlsx`;
    link.click();
  ...
  async function exportPdf() {
    ...
    try {
      ...
      doc.text(`Overtimes de ${MONTH_NAMES[month - 1]} de ${year}`, textX, 23.5);
  ...
  function changeTotalSum(newTotalSum: number) {
    setReports((prev) =>
      prev.map((report) =>
        report.user_id === userSelected?.id
          ? { ...report, minutesInNumber: Number(newTotalSum), minutes: formatMinutes(newTotalSum) }
          : report
      )
    );
  }
  ...
  return (
    <>
            ...
            <DataTable
              ...
              emptyMessage={
                <div className="license-empty">
                  ...
                  <p>No hay overtimes para el mes y año</p>
                </div>
              }
            >
        ...
        header={
          <div className="d-flex align-items-center" style={{ gap: "12px" }}>
            ...
            <div style={{ minWidth: 0 }}>
              <p className="mb-0 font-weight-bold" style={{ fontSize: "0.93rem", color: "#1e293b" }}>Overtimes</p>
      ...
      >
        {pdfUrl && <iframe src={pdfUrl} style={{ flex: 1, width: "100%", border: "none" }} title="Overtimes PDF" />}
      </Sidebar>

      <OvertimesDetailsDialog
        visible={isOpenModalDetails}
        onHide={() => setIsOpenModalDetails(false)}
        user={userSelected}
        year={year}
        month={month}
        onChangeTotalSum={changeTotalSum}
      />
    </>
  );
}
