...
import { Hourglass, FileSpreadsheet, FileText, Search, List, Plus, Pencil, XCircle } from "lucide-react";
...
function normalizeReport(report: any) {
  return {
    ...report,
    minutesInNumber: Number(report.work_minutes) || 0,
    minutes: formatMinutes(report.work_minutes),
  };
}
...
export default function HorasExtrasPage() {
  ...
  const [horaExtraSelected, setHoraExtraSelected] = useState<any>(null);
  ...
  async function loadReports() {
    ...
    setNameFilter("");
    try {
      const resp = await loadAllHorasExtras({ year, month });
      const data = Array.isArray(resp?.data) ? resp.data : [];
      setReports(data.map(normalizeReport));
    } catch (err: any) {
      toast.current?.show({ severity: "error", summary: "No se pudieron cargar las horas extras", detail: err.message });
    } finally {
      setIsLoading(false);
    }
  }
  ...
  function buildExportRows() {
    return filtered.map((item) => ({
      ...
      "Fechas computadas": item.begin_date ? `${formatDate(item.begin_date)} a ${formatDate(item.end_date)}` : "--",
      "Turno": item.shift ?? "--",
    }));
  }
  ...
  async function exportExcel() {
    ...
    const headers = rows.length > 0 ? Object.keys(rows[0]) : ["Agente", "Suma total", "Fechas computadas", "Turno"];
    ...
    link.href = url;
    link.download = `Horas extras de ${MONTH_NAMES[month - 1]} de ${year}.xlsx`;
    link.click();
  ...
  async function exportPdf() {
    ...
    try {
      ...
      doc.text(`Horas extras de ${MONTH_NAMES[month - 1]} de ${year}`, textX, 23.5);
      ...
      autoTable(doc, {
        startY: 35,
        head: [["Agente", "Suma total", "Fechas computadas", "Turno"]],
  ...
  function openModalCreate(horaExtra: any) {
    setHoraExtraSelected(horaExtra);
    setIsOpenModalCreate(true);
  }

  function openModalDetails(horaExtra: any) {
    setHoraExtraSelected(horaExtra);
    setIsOpenModalDetails(true);
  }

  function openModalDelete(horaExtra: any) {
    setHoraExtraSelected(horaExtra);
    setIsOpenModalDelete(true);
  }

  function updateHoraExtra(horaExtraModificada: any) {
    setReports((prev) =>
      prev.map((horaExtra) =>
        horaExtra.id === horaExtraModificada.id
          ? { ...horaExtra, work_planner_type_id: horaExtraModificada.work_planner_type_id, begin_date: horaExtraModificada.begin_date, end_date: horaExtraModificada.end_date, shift: horaExtraModificada.shift }
          : horaExtra
      )
    );
  }
  ...
  return (
    <>
      ...
      <div className="fadeIn animated">
        ...
        <div className="card profile-card">
          <div className="d-flex align-items-center flex-wrap px-3 pt-3 pb-3" style={{ gap: "12px" }}>
            <div style={{ width: 38, height: 38, borderRadius: "11px", background: "#fef9c3", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
              <Hourglass size={18} color="#eab308" />
            </div>
            <div className="flex-grow-1">
              <h5 className="mb-0 font-weight-bold" style={{ fontSize: "0.93rem", color: "#1e293b" }}>Horas extras</h5>
              <small style={{ color: "#94a3b8", fontSize: "0.75rem" }}>Planificación de horas extra por agente</small>
            </div>
        ...
        <div className="card profile-card mt-4">
          <div className="d-flex align-items-center px-3 pt-3 pb-2" style={{ gap: "12px" }}>
            ...
            <div className="flex-grow-1">
              <h5 className="mb-0 font-weight-bold" style={{ fontSize: "0.93rem", color: "#1e293b" }}>Buscar horas extras</h5>
        ...
        <div className="card profile-card mt-4">
          <div className="d-flex align-items-center px-3 pt-3 pb-2" style={{ gap: "12px" }}>
            ...
            <div className="flex-grow-1">
              <h5 className="mb-0 font-weight-bold" style={{ fontSize: "0.93rem", color: "#1e293b" }}>Listado de horas extras</h5>
              <small style={{ color: "#94a3b8", fontSize: "0.75rem" }}>Horas extra planificadas por agente</small>
            </div>
            <button
              type="button"
              disabled={isLoading}
            ...
            <DataTable
              ...
              emptyMessage={
                <div className="license-empty">
                  ...
                  <p>No hay horas extras para el mes y año</p>
                </div>
              }
            >
              <Column header="AGENTE" sortable field="lastname_name" body={(item) => <small>{item.lastname_name}</small>} />
              <Column header="SUMA TOTAL" sortable field="minutesInNumber" style={{ width: "14%" }} body={(item) => <small className="p-1">{item.minutes}</small>} />
              <Column
                header="FECHAS COMPUTADAS"
                body={(item) => <small>{item.begin_date ? `${formatDate(item.begin_date)} a ${formatDate(item.end_date)}` : "--"}</small>}
              />
              <Column header="TURNO" style={{ width: "12%" }} body={(item) => <small>{item.shift ?? "--"}</small>} />
              <Column
                header=""
                style={{ width: "14%" }}
                body={(item) => (
        ...
        header={
          <div className="d-flex align-items-center" style={{ gap: "12px" }}>
            ...
            <div style={{ minWidth: 0 }}>
              <p className="mb-0 font-weight-bold" style={{ fontSize: "0.93rem", color: "#1e293b" }}>Horas extras</p>
      ...
      >
        {pdfUrl && <iframe src={pdfUrl} style={{ flex: 1, width: "100%", border: "none" }} title="Horas extras PDF" />}
      </Sidebar>

      <HorasExtrasCreateDialog
