...
import { createCompensation } from "@/lib/services/compensations.service";

addLocale("es-compensations-create", {
  firstDayOfWeek: 1,
...
const EMPTY_FORM = { begin_date: "", end_date: "", shift: "", user_id: "", work_planner_type_id: "1" };
...
export default function CompensationsCreateDialog({ visible, onHide, compensation, year, month, onCreated }: Props) {
  const toast = useRef<Toast>(null);
  const [form, setForm] = useState(EMPTY_FORM);
  const [isCreating, setIsCreating] = useState(false);

  useEffect(() => {
    if (compensation?.user_id) setForm((p) => ({ ...p, user_id: compensation.user_id }));
  }, [compensation]);
  ...
  const canCreate = useMemo(() => {
    return !!(form.shift && form.begin_date && form.end_date);
  }, [form]);

  function resetForm() {
    setForm({ ...EMPTY_FORM, user_id: compensation?.user_id ?? "" });
  }
  ...
  const dialogHeader = (
    <div className="d-flex align-items-center" style={{ gap: "12px" }}>
      ...
      <div style={{ minWidth: 0 }}>
        <p className="mb-0 font-weight-bold" style={{ fontSize: "0.93rem", color: "#1e293b" }}>Crear compensación</p>
          ...
          <div className="col-12 col-md-6 mb-3">
            ...
            <div className="license-filter-input-wrap profile-birthdate-wrap">
              ...
              <Calendar
                ...
                dateFormat="dd/mm/yy"
                locale="es-compensations-create"
